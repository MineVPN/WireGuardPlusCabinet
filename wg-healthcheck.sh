...
#     не только на /24.
#
# Синхронизация с панелью: /var/www/wgplus/state (STATE=running|stopped|busy).
...
HEALTH_FILE="/var/www/wgplus/health"
...
POLL_MAX=8               # макс итераций ожидания подъёма после перезапуска.
                         # Итерация — это sleep 1 плюс проверка связи каждую
                         # третью, то есть реально около 12 секунд. На это время
                         # цикл заблокирован: не видно ни падения, ни смены
                         # настроек в панели — поэтому не затягиваем.
...
# Приоритет временного правила проверки связи.
#
# Обязан быть НИЖЕ локального правила (100), а не выше. Правило задаётся
# как 'from <адрес шлюза>' без ограничения по назначению, поэтому с
# приоритетом 90 под него попадали и ответы сервера клиентам (источник
# 10.55.55.1, назначение 10.55.55.x) — и уходили в туннель вместо wg0.
# Пока второй впн лежал, проверка шла почти непрерывно, и клиенты всё
# это время не могли открыть панель по адресу шлюза и пинговать шлюз.
PREF_PROBE=110
PREF_BYPASS=30000   # обход VPN; меньше 32765, поэтому срабатывает раньше цепочки
...
#
# Возврат по ПЕРВОМУ успеху, а не по завершении всех пяти.
# Раньше здесь стоял 'wait' на все процессы сразу: если хотя бы один адрес
# из списка не маршрутизируется (9.9.9.9 и 1.0.0.1 режут довольно часто),
# каждая проверка стоила полный PING_TIMEOUT даже при живом туннеле —
# период цикла удваивался, а ко времени переключения добавлялось до
# четырёх секунд на двух пробах плюс ещё две на проверке WAN.
ping_via() {
    local via="$1"
    [ -z "$via" ] && return 1
    local flag
    ...
    # Ждём максимум PING_TIMEOUT, проверяя флаг десять раз в секунду.
    ...
    while [ "$waited" -lt "$limit" ]; do
        ...
        # отвалившийся ping (например, «Network is unreachable») оборвал бы
        # ожидание, пока остальные ещё работают.
        alive=0
        for p in "${pids[@]}"; do
...
# Проверка канала данных без ICMP.
#
# curl --interface от root использует SO_BINDTODEVICE: пакет уходит прямо
# в указанный интерфейс мимо таблиц маршрутизации. Это проверка именно
# туннеля, а не правил — и она работает одинаково в обычном режиме и
# в режиме обхода, без временного правила проверки.
# Только IP-адреса, без доменных имён: DNS сервера может смотреть куда угодно
# (в том числе во второй впн), и разрешение имени добавило бы к проверке
...
#
# 'curl --interface wg1' не обходит маршрутизацию: SO_BINDTODEVICE лишь
# ограничивает поиск маршрута этим устройством, а сам поиск идёт как обычно.
# Маршрут по умолчанию через wg1 лежит ТОЛЬКО в таблице 120, попасть в неё
# можно исключительно по правилу 'from <подсеть wg0>', и в main его нет
# (wg1.conf ставится с Table = off). Такой запрос упёрся бы в ENETUNREACH,
# то есть проверка всегда была бы отрицательной.
#
# С адресом шлюза (он лежит в подсети wg0) пакет попадает под то же правило
# policy routing, что и трафик клиентов, — ровно как в ping_tunnel.
http_via_gw() {
    local ip
    for ip in "${HTTP_PROBE_IPS[@]}"; do
        curl -sS --connect-timeout 2 --max-time 2 --interface "$WG0_GW" \
...
# шлюза (один /32) с приоритетом PREF_PROBE. Клиентский трафик не затрагивается:
...
probe_tunnel() {
    ...
    # обязаны идти по одному и тому же пути. Раньше правило снималось
    # сразу после ping, и запасная проверка уходила бы уже мимо туннеля.
    if [ "${CHAIN_BYPASSED:-0}" -eq 1 ]; then
        ip rule add from "$WG0_GW" table "$TABLE_ID" preference "$PREF_PROBE" 2>/dev/null && added=1
    fi
    ...
    [ "$added" -eq 1 ] && \
        ip rule del from "$WG0_GW" table "$TABLE_ID" preference "$PREF_PROBE" 2>/dev/null

    return "$rc"
}

# ПОЧЕМУ ЗДЕСЬ БОЛЬШЕ НЕТ ПРОВЕРКИ ВОЗРАСТА HANDSHAKE.
#
# Условие 'age < 60' объявляло туннель живым, даже когда все пять пингов
# провалились. Это давало два дефекта сразу:
#
#   1. Задержку переключения от нуля до шестидесяти секунд. WireGuard
#      обновляет handshake примерно раз в две минуты, поэтому в момент
#      аварии его возраст — случайная величина, и всё это время упавший
#      туннель считался живым, а клиенты сидели без интернета.
#
#   2. Флаппинг с периодом около двух минут, когда узел второго впн
#      отвечает на handshake, но трафик не пропускает (у продавца
#      кончилась подписка — самый частый сценарий отказа). Handshake
#      успешно обновлялся, возраст возвращался к нулю, туннель объявлялся
#      живым, правило цепочки возвращалось — и интернет пропадал снова.
#
# Запасная проверка по HTTP отвечает на правильный вопрос: «ходит ли
# трафик», а не «отвечает ли узел на служебные пакеты».
#
# handshake_age() оставлена — она используется в note_down() для журнала.
...
rules_order_ok() {
    ...
    # Цепочки нет — упорядочивать нечего, и это штатное состояние,
    # пока второй впн не загружен. Возвращать здесь «неверный порядок»
    # значило бы звать fix_rules_order на каждом старте: он писал бы
    # тревожный WARN на исправной системе и добавлял правило цепочки
    # при отсутствующем wg1.
    [ -z "$chain" ] && return 0
    [ -z "$loc" ] && return 1
...
enter_failover() {
    ...
    if has_chain_rule; then
        # Удаляем БЕЗ указания preference.
        #
        # Условие цикла ловит правило с любым приоритетом, а удаление раньше
        # шло строго по 32765. Правило с другим приоритетом (после
        # fix_rules_order, kill_orphan или ручной правки) не удалялось,
        # '|| break' молча выходил, а флаг всё равно выставлялся — клиенты
        # оставались без интернета, и демон больше не пытался это исправить.
        local i=0
...
# ══════════════════════════════════════════════════════
#
# ЗАЧЕМ: панель определяла состояние туннеля одним лишь наличием
# интерфейса. Во время обхода интерфейс есть, поэтому она показывала
# «Подключено. Выход мимо туннеля закрыт» — ровно тогда, когда трафик
# на самом деле шёл напрямую с адреса сервера. Единственный индикатор
# для владельца врал именно в момент аварии.
#
...
mode_rules_ok() {
    # Пустой WAN_IF — это НЕ «всё в порядке».
    #
    # Раньше здесь стоял return 0, и при неопределённом интерфейсе цикл
    # считал режим исправным навсегда: apply_forward_mode один раз писал
    # ERR при старте и больше не вызывался. А поскольку политика FORWARD
    # остаётся ACCEPT, отсутствие правил означает «пропускать всё» —
    # Kill Switch молча не работал, и утечка была невидимой.
    if [ -z "$WAN_IF" ]; then
        load_nic
        [ -z "$WAN_IF" ] && return 1
    fi
...
check_forward_mode() {
    ...
    # не выполнялась вовсе, из-за чего Kill Switch молча не работал.
    # Пробуем определить интерфейс заново, и только потом сдаёмся.
    if [ -z "$WAN_IF" ]; then
        load_nic
...
sync_bypass() {
    ...
    iptables_available || return 0
    now=$(date +%s)
    # Пять секунд, а не десять: apply_forward_mode сбрасывает BYPASS_HASH,
    # и до пересборки цепочки адреса обхода шли через туннель. Десять
    # секунд такого поведения после каждой смены режима — заметно.
    ...
    # и первый проход выходил рано, не создав цепочку WGPLUS_BYPASS вовсе.
    ...
    if [ -f "$ROUTES_FILE" ]; then
        while IFS= read -r ip || [ -n "$ip" ]; do
            ...
            iptables -A WGPLUS_BYPASS -d "$ip" -j ACCEPT 2>/dev/null
            # Правило маршрутизации ставит ДЕМОН, а не панель.
            #
            # Раньше это делала панель через sudo, для чего в sudoers стоял
            # шаблон 'ip rule add to * table main preference 30000'. Звёздочка
            # там матчит и слэш, поэтому одна команда 'to 0.0.0.0/0' уводила
            # ВЕСЬ трафик клиентов мимо цепочки. Правило из sudoers убрано.
...
restart_tunnel() {
    ...
    while [ $i -lt "$POLL_MAX" ]; do
        # ПЕРВЫМ ДЕЛОМ, до сна и до проверки.
        #
        # PostUp из wg1.conf возвращает правило цепочки сразу при запуске,
        # ещё до того как туннель заработал. Раньше снятие стояло в конце
        # тела цикла — то есть после sleep 1 и полной проверки связи, — и
        # каждая попытка восстановления отбирала у клиентов интернет
        # на пару секунд. При долгой аварии таких попыток десятки.
        [ "${CHAIN_BYPASSED:-0}" -eq 1 ] && enter_failover

        sleep 1; i=$((i + 1))

        # Дешёвые проверки на каждой итерации, полная — каждую третью.
        # Полная стоит до нескольких секунд, и на каждом шаге растягивала бы
        # цикл втрое: всё это время демон не читает состояние и не соблюдает
        # флаг busy, выставленный панелью.
        if iface_exists && iface_has_ip && has_chain_route; then
            if [ $((i % 3)) -eq 0 ] || [ "$i" -eq "$POLL_MAX" ]; then
...
main_loop() {
    ...
    while true; do
        ...
        # через две минуты начинает показывать «демон не отвечает» при
        # полностью рабочей системе. Панели нужна свежесть, а не событие.
        write_health
        ...
        # Жив ли САМ СЕРВЕР для клиентов — проверяем ДО всех выходов из цикла.
        #
        # Во-первых, весь каскад ниже про wg1: если упал wg0, ping -I <адрес
        # шлюза> падает мгновенно с «Cannot assign requested address», демон
        # ставит диагноз «нет связи через туннель» и до бесконечности
        # перезапускает исправный wg1, не трогая настоящую причину.
        #
        # Во-вторых, в режиме direct (второго впн нет) до проверок дело вообще
        # не доходило бы — а упавший wg0 там ровно так же означает, что
        # клиенты отключены.
        if ! ip link show wg0 &>/dev/null \
           || ! ip -4 addr show wg0 2>/dev/null | grep -q "inet ${WG0_GW}\b"; then
            note_down "не поднят интерфейс wg0 — второй впн ни при чём"
            vpn_ok=0
            # С паузой между попытками: без неё при по-настоящему битом
            # wg0.conf демон дёргал бы systemd каждые две секунды и упёрся
            # в его ограничение частоты запусков, а note_down показал бы
            # оператору всего одну строку в пять минут.
            now=$(date +%s)
            if [ $((now - WG0_LAST_TRY)) -ge 30 ]; then
                WG0_LAST_TRY=$now
                log "WARN" "Пробую поднять wg0"
        ...
        # tunnel_alive, а НЕ probe_tunnel.
        #
        # Разница принципиальная и описана в комментарии к tunnel_alive:
        # маршрут в таблице 120 обязан проверяться ДО пинга. Если маршрут
        # пропал, пакет проваливается в main, уходит через NIC под
        # MASQUERADE — и пинг успешен при мёртвом туннеле. Демон рапортует
        # «Туннель работает», а весь трафик клиентов в это время идёт
        # с реального адреса сервера.
        #
